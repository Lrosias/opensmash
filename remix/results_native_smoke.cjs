 ...
 try{
  ...
  for(const id of roster){
   const result=await p.evaluate(id=>{
    ...
    for(let pose=0;pose<3;pose++){
     ...
     pads=[[0,0,0],[0,0,0]];for(let n=0;n<150;n++)driver.step();
     const before=probe();for(let n=0;n<260;n++)driver.step();
  ...
  for(let round=0;round<3;round++){
   const result=await p.evaluate(()=>{
    ...
    if(m.nativeScene!==24)throw Error('Match did not finish');tick(410);return {...m.remixResults,frames};
