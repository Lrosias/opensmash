...
(async()=>{const browser=await chromium.launch({headless:true,executablePath:process.env.CHROMIUM_PATH,args:['--no-sandbox','--enable-unsafe-swiftshader']});try{const report={};for(const name of ['charge_ground','charge_air','boost_ground','boost_air']){
 ...
 for(let i=0;(state?.[4]||0)<30&&i<900;i++)tick(1);if(!game.contentWindow.Module._port_remix_test_place(650))throw Error('place');tick(2);rows=[];entities=[];
 ...
 if(name==='boost_ground'){tick(1,[16384,0,-70]);tick(180,[0,70,0]);tick(100);}
 ...
 assert(report.boost_air.statuses.includes(232)&&report.boost_air.statuses.includes(233),'Aerial Boost Ball did not land');assert(report.boost_ground.damage>0,'Boost Ball never hit');
