...
for(const seat of [0,1])for(const mode of ['walkoff','tap','charge','air','landing','grenade','grenade-air']){
 ...
 const r=await Promise.race([page.evaluate(async({seat,mode})=>{
  ...
  if(mode!=='walkoff'){m._port_remix_test_place(1800);tick();if(mode==='air'||mode==='landing'||mode==='grenade-air')m._port_remix_test_position(seat,seat?900:-900,mode==='landing'?300:1800);tick();}
  ...
  const input=n=>mode==='walkoff'?(n<36?[0,seat?80:-80,0]:n===37?[16384,0,0]:[0,0,0]):[n<(mode==='charge'||mode==='landing'?80:mode==='air'?12:mode.startsWith('grenade')?12:1)?16384:0,0,mode.startsWith('grenade')?-70:0];
  const store=new NativeCheckpoints(driver,{window:240}),snap=store.save(0,state),rows=[];for(let n=0;n<220;n++){tick(input(n));rows.push(observe());}
  state=store.load(snap);for(let n=0;n<220;n++){tick(input(n));if(JSON.stringify(observe())!==JSON.stringify(rows[n]))throw Error('rollback divergence '+mode+' '+seat+' '+n);}
 ...
 assert.deepEqual(errors,[]);if(['tap','charge','air','landing'].includes(mode))assert(r.maxWeapons>0,`${mode} emitted no projectile`);if(mode.startsWith('grenade'))assert(r.maxItems>0,`${mode} emitted no grenade`);results.push(r);console.log(JSON.stringify(r));await fs.writeFile('build/remix/conker-fix/regression.json',JSON.stringify(results,null,2));await page.close();
}
