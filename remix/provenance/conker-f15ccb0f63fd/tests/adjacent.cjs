...
(async()=>{const browser=await chromium.launch({headless:true,executablePath:process.env.CHROMIUM_PATH,args:['--no-sandbox','--enable-unsafe-swiftshader','--mute-audio']});try{
 ...
 await page.goto('http://127.0.0.1:4207/remix-check.html?battle='+ (process.env.FIGHTER||58) +',74,16,0&SSB64_REMIX_TEST=1&SSB64_YOUGAME_MUTE=1');await page.waitForFunction(()=>window.driver,{},{timeout:60000});
 ...
 r.errors=errors;await fs.writeFile(`build/remix/conker-fix/specials-${process.env.FIGHTER||58}.json`,JSON.stringify(r));console.log(Object.fromEntries(Object.entries(r).map(([k,v])=>[k,Array.isArray(v)&&v[0]?.length===32?{statuses:[...new Set(v.map(x=>x[1]))],damage:[v[0][2],v.at(-1)[2]],foeDamage:[v[0][18],v.at(-1)[18]],ymax:Math.max(...v.map(x=>x[4]))}:v])));await page.screenshot({path:`build/remix/conker-fix/specials-${process.env.FIGHTER||58}.png`});
