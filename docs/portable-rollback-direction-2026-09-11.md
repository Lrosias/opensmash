...
The Melee native-menu investigation found a concrete emulator-device issue: Dolphin's memory-card initialization paths derive header time, serial and checksums from the host clock even when the emulated RTC is fixed. Two peers first diverged in 17 bytes of a header during native post-results progression. Fixing only the raw-card constructor did not resolve the runtime failure: the default slot uses a folder-backed card initialized separately in EXI. Both creation paths need the emulated clock. Existing-card reads and offline behavior retain their current paths. Compiled fixtures verify the patched expressions across differing host times; full native lifecycle verification remains a separate gate.
