...
The Melee native-menu investigation found a concrete emulator-device issue: newly formatted Dolphin raw memory cards derive their header time, serial and checksums from the host clock even when the emulated RTC is fixed. Two peers first diverged in 17 bytes of that header during native post-results progression. The browser rollback adapter now uses Dolphin's emulated clock for new-card formatting; existing-card reads and offline behavior retain their current paths. A compiled fixture verifies the patched expression across differing host times. Full native lifecycle verification is recorded separately.
