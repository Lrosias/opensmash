...
try{browser=await chromium.launch({headless:true,timeout:15000,args:['--mute-audio']});timer=setTimeout(()=>browser.close(),90000);
 for(const edition of ['original','remix']){
  ...
  await page.evaluate(()=>{nativeRoomFixture.terminal=true;nativeRoomFixture.syncs[0].advance();});await page.waitForFunction(()=>nativeRoomFixture.syncs[1]?.running);
  assert.deepEqual(await page.evaluate(()=>nativeRoomFixture.finishes),[{winner:'p3'}]);
  ...
  await page.screenshot({path:path.join(out,`${edition}-native-result.png`)});
  ...
  evidence.push({edition,noCustomSelection:true,startEnabled:true,globalPortHoles:true,sameEngineResultContinuation:true,leaveDisposesSession:true,errors});await page.close();
 }
