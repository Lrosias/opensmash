const menuFrame=p=>p.locator('iframe[title="OpenSmash native menus"]');
const battleFrame=p=>p.locator('iframe[title="OpenSmash online battle"]');
const scene=(p,n)=>p.waitForFunction(n=>document.querySelector('iframe[title="OpenSmash native menus"]')?.contentWindow.Module?.nativeScene===n,n,{timeout:90000});
...
async function chooseFighter(p,seat=0){
 ...
 await key(p,'ArrowUp',450);await key(p,'ArrowRight',seat?280:100);await key(p,'KeyM');await p.waitForTimeout(400);
 await key(p,'Enter');
