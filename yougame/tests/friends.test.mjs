...
test('Friends owns Ready and Continue through the SDK without public fill',async()=>{
 const t=await setup();await t.queue('friends');const call=t.calls[0],r=room();assert.equal(call.options.queue,'friends');call.options.onStatus({room:r});assert.equal(r.readyCalls,0);assert.deepEqual(JSON.parse(JSON.stringify(r.settingsCalls)),[{fill:false}]);assert.equal(t.sets.length,0);assert.equal(t.bridge.beforeTick(),false);
