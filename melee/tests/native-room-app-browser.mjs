...
window.createMelee=async options=>{
 window.localFixture={frames:0,paused:false,transitions:[]};
 ...
 return {_melee_prepare_assets:()=>0,_melee_shader_cache_format:()=>0,_melee_audio_ring:()=>0,
  ...
  _melee_frame_ack(){},_melee_input(){},ccall(){},FS:{readFile(){throw Error('empty');}},
...
try{
 for(const mode of ['casual','friends','invite','cancel','ranked','engine-failure','close']){
  ...
  const kind={casual:0,ranked:1,friends:2,cancel:0,'engine-failure':0,close:0}[mode];
  ...
  if(mode==='cancel'){
   ...
   const queue=mode==='invite'?'friends':mode==='close'?'casual':mode;
   ...
   if(mode==='ranked')await page.keyboard.press('Escape');else if(mode==='close')await page.evaluate(()=>fixtureRoom.emit('close',{}));else await page.evaluate(()=>melee.menuAction(2,0));
   ...
   assert.deepEqual(await page.evaluate(()=>localFixture.transitions),[true,false]);
   assert.equal(await page.evaluate(()=>melee.menu.phase),mode==='ranked'||mode==='close'?6:0);
  }
