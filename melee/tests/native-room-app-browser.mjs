...
const server=createServer(async(req,res)=>{try{const name=path.join(root,new URL(req.url,'http://test').pathname);if(!name.startsWith(root+'/'))throw Error('path');const data=await readFile(name);res.setHeader('Cross-Origin-Opener-Policy','same-origin');res.setHeader('Cross-Origin-Embedder-Policy','credentialless');res.setHeader('content-type',name.endsWith('.mjs')||name.endsWith('.js')?'text/javascript':name.endsWith('.css')?'text/css':name.endsWith('.html')?'text/html':'application/octet-stream');res.end(data);}catch{res.statusCode=404;res.end();}});
...
port.onmessage=({data})=>{let value={};if(data.method==='boot'){if(!data.args[0].nativeSession)throw Error('Expected a native session');mask=data.args[0].slots.reduce((m,s)=>m|(1<<s),0);parent.fixtureLaunch=data.args[0];}
...
try{
 for(const mode of ['casual','friends','invite','cancel','ranked']){
  ...
  // The local engine never finishes booting here: the page keeps its build identity and the online flow.
  await context.route('**/engine/melee.js',r=>r.fulfill({contentType:'text/javascript',body:'window.createMelee=()=>new Promise(()=>{});'}));
  ...
  await page.goto(url);await page.waitForFunction(()=>melee.build&&melee.phase==='loading');
  ...
  const kind={casual:0,ranked:1,friends:2,cancel:0}[mode];
  ...
  if(mode==='cancel'){
   ...
   assert.equal(call.mode,'opensmash-melee-native-menu-v1');assert.equal(call.queue,mode==='invite'?'friends':mode);assert.equal(call.players,mode==='ranked'||mode==='casual'?2:4);assert.equal(call.maxLocalPlayers,mode==='ranked'||mode==='casual'?1:4);
   ...
   if(mode==='ranked')await page.keyboard.press('Escape');else await page.evaluate(()=>melee.menuAction(2,0));
   ...
   assert.equal(await page.evaluate(()=>melee.menu.phase),mode==='ranked'?6:0);
  }
